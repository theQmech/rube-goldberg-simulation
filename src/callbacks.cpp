...
void callbacks_t::display_cb(void) {
  ...
  if (old_center.x != settings.view_center.x ||
      old_center.y != settings.view_center.y) {
    resize_cb(width, height);
  }
...
void callbacks_t::pause_cb(int) { settings.pause = !settings.pause; }

void callbacks_t::exit_cb(int code) { exit(code); }
